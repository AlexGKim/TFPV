...
probailistically based on this parameter using the TF relation.  Meaurement errors are
independent.  Specifically, for each galaxy $i$:
\begin{align}
...
The velocity uncertainties are normal in log-space, which is probably not entirely accurate.
\subsection{Complete Sample}
It is useful to go through the case of a complete sample, even though that is not the case for the DESI TF sample.
...
\end{align}
Without the CDF factor, $\sigma_{{\rm int},x}$ and $\sigma_{{\rm int},y}$ are degenerate and cannot be determined
independently.
...
the SGA selection is independent of the TF observables.  In particular, sample selection does not depend on observed magnitude.
Concretely, letting $S_{\rm SGA}\in\{0,1\}$ denote
...
overall constant to the likelihood and therefore does not effect TF parameter estimation.
...
The expected number of sample galaxies, $\bar N$ is then calculablable from the model and is dependent
on the TF parameters. The actual sample size can then provide additional information and help constrain the TF parameters.
...
As a result, the TF parameters are constrained only by the observed distribution of $(\hat x,\hat y)$.
...
\end{align}
There $y_{\max}$ is meant to eliminate ``all'' of the dwarf galaxies from the sample.
Selection will be galaxy-independent though this doesn't have to be the case in general.
...
This case is implemented in {\tt base.stan} with {\tt int y\_TF\_limits = 1};
  {\tt y\_selection = 1}; {\tt plane\_cut = 0;}and when run with Ariel's mock after application of
a maximum $\hat{y}_{\max}$
  the input $s$ and $c$ are recovered to incredible precision,
  and the intrinsic dispersions to 1$\sigma$.
...
\ \ \Longleftrightarrow\ \
\bar{s}\,\hat x_i+\bar{c} \le \hat y_i \le \hat y_{\max, i},
\end{align}
...
\begin{equation}
\hat y \le \hat y_{\max,i}.
\end{equation}
Therefore the selection region is
\begin{equation}
\mathcal S_i=\{(\hat u,\hat y):\ \hat u\ge 0,\ \hat y\le \hat y_{\max,i}\}.
\end{equation}
...
P(S_i=1\mid\theta)
& =
\int dy_{\mathrm{TF},i}\; p(y_{\mathrm{TF},i})
\int_{-\infty}^{\hat y_{\max,i}} d\hat y
\int_{0}^{\infty} d\hat u\;
\frac{1}{|\bar{s}|}\,
\mathcal N\!\left(\frac{\hat y-\bar{c}-\hat u}{\bar{s}};\frac{y_{\mathrm{TF},i}-c}{s},\sigma_{1,i}\right)\,
\mathcal N\!\left(\hat y;y_{\mathrm{TF},i},\sigma_{2,i}\right)
\end{align}
...
\end{equation}
\begin{equation}
\beta = \frac{\hat y_{\max,i} - y_{\mathrm{TF},i}}{\sigma_{2,i}}
\end{equation}
\begin{align}
P(S_i=1\mid\theta)
& =
...
This integral has no analytic solution and is integrated numerically.

For the case with limits this is
\begin{align}
P(S_i=1\mid\theta)
& = \frac{1}{y_{max}-y_{min}}
\int_{y_{min}}^{y_{max}} dy_{\mathrm{TF},i}
\Phi_2\left(-\alpha, \beta; -\rho \right)
\end{align}

This case is implemented in {\tt base.stan} with {\tt int y\_TF\_limits = 1};
  {\tt y\_selection = 1}; {\tt plane\_cut = 1;}and when run with Ariel's mock after application of
a maximum $\hat{y}_{\max}$
  the input $s$ and $c$ are recovered tp incredible precision,
  and the intrinsic dispersions to 1$\sigma$ in preliminary tests.


\paragraph{With a two-sided half-plane cut (parallel case, $\bar{s}_1=\bar{s}_2\equiv\bar{s}$)}

When the two half-plane boundaries are parallel, i.e.\
$\bar{s}_1=\bar{s}_2\equiv\bar{s}$, the selection domain is
\begin{equation}
\mathcal S_i:\qquad
\bar{s}\,\hat x_i+\bar{c}_1 \;\le\; \hat y_i \;\le\; \min\!\big\{\hat y_{\max,i},\;\bar{s}\,\hat x_i+\bar{c}_2\big\}.
\end{equation}
...
integrals:
\begin{equation}
P(\hat u\ge0,\;\hat u\le\bar{c}_2-\bar{c}_1,\;\hat y\le\hat y_{\max,i})
\;=\;
P(\hat u\ge0,\;\hat y\le\hat y_{\max,i})
\;-\;
P(\hat u\ge\bar{c}_2-\bar{c}_1,\;\hat y\le\hat y_{\max,i}).
\end{equation}
Each term is a bivariate normal CDF of the same form as in
Section~\ref{subsec:selection_yhatmax_halfplane}. Defining
\begin{equation}
\rho \;=\; \frac{\sigma_{2,i}}{\sqrt{\sigma_{2,i}^2+\bar{s}^2\,\sigma_{1,i}^2}},
\end{equation}
\begin{equation}
...
For a uniform prior $p(y_{\mathrm{TF},i})=1/(y_{\max}-y_{\min})$ this becomes
\begin{align}
P(S_i=1\mid\theta)
&=\frac{1}{y_{\max}-y_{\min}}
\int_{y_{\min}}^{y_{\max}} dy_{\mathrm{TF},i}\;
