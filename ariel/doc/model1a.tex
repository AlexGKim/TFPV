...
The integral is not solved analytically.  There may be an efficient way to solve this numerically.
...
\int_{0}^{\infty} d\hat u\;
\frac{1}{|a|}\;
\mathbb{E}_{\,y_{\mathrm{TF},i}\sim \mathcal N(\hat y,\sigma_{2,i})}
\!\left[
\mathcal N\!\left(
\frac{\hat y-b-\hat u}{a}\;;\;\frac{y_{\mathrm{TF},i}-c}{s},\sigma_{1,i}
\right)
\right].
\end{align}

\paragraph{Case of limits on $y_{\rm TF}$}
Consider
\begin{equation}
p\!\left(y_{\mathrm{TF},i}\right)=
\begin{cases}
\dfrac{1}{y_{\max}-y_{\min}}, & y_{\min}\le y_{\mathrm{TF},i}\le y_{\max},\\[6pt]
0, & \text{otherwise.}
\end{cases}
\end{equation}
