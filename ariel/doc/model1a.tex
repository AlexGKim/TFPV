...
a maximum $\hat{y}_{\max}$
  ...
  and the intrinsic dispersions to 1$\sigma$.  Note that the analysis model for $p(y_{TF})$ is inconsistent
  with the mock so some deviation is to be expected.
...
\ \ \Longleftrightarrow\ \
a\,\hat x_i+b \le \hat y_i \le \hat y_{\max, i},
\end{align}
where $(a,b)$ are fixed constants defining the half-plane boundary, introduce the sheared coordinates $(\hat u,\hat y)$ by defining
\begin{equation}
\hat u \equiv \hat y - a\,\hat x - b,
\qquad\text{and}\qquad
\hat y \equiv \hat y.
\end{equation}
In these coordinates, the oblique constraint becomes the constant bound
\begin{equation}
\hat y-a\,\hat x-b \ge 0 \quad\Longleftrightarrow\quad \hat u \ge 0,
\end{equation}
...
\begin{equation}
\hat x = \frac{\hat y-b-\hat u}{a},
\qquad
\hat y=\hat y,
\end{equation}
and the Jacobian determinant is constant:
\begin{equation}
\left|\frac{\partial(\hat x,\hat y)}{\partial(\hat u,\hat y)}\right|=\frac{1}{|a|},
\qquad\text{so}\qquad
d\hat x\,d\hat y = \frac{1}{|a|}\,d\hat u\,d\hat y.
\end{equation}
...
\frac{1}{|a|}\,
\mathcal N\!\left(\frac{\hat y-b-\hat u}{a};\frac{y_{\mathrm{TF},i}-c}{s},\sigma_{1,i}\right)\,
\mathcal N\!\left(\hat y;y_{\mathrm{TF},i},\sigma_{2,i}\right)
\end{align}

Given the parameterization
\begin{equation}
\rho \;=\; \frac{\sigma_{2,i}}{\sqrt{\sigma_{2,i}^2 + a^2\,\sigma_{1,i}^2}}
\end{equation}
\begin{equation}
\alpha =
\frac{b - \left(y_{\mathrm{TF},i}-a\,\frac{y_{\mathrm{TF},i}-c}{s}\right)}{\sqrt{\sigma_{2,i}^2 + a^2\,\sigma_{1,i}^2}}
\end{equation}
\begin{equation}
...
a maximum $\hat{y}_{\max}$
 ...
 Note that the analysis model for $p(y_{TF})$ is inconsistent
  with the mock so some deviation is to be expected.
