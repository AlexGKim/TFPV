...
`SGA-2020_iron_Vrot_VI_corr_v6.fits` dataset.

## Setup

```bash
export FITS=data/SGA-2020_iron_Vrot_VI_corr_v6.fits   # input FITS catalog
export RUN=DR1_v6                                         # output directory name: output/$RUN/
export CONFIG=configs/dr1_v6.json                      # pipeline config (parameters + file paths)
```
...
```bash
python export_config.py --run $RUN --out configs/dr1_v6.json
```
