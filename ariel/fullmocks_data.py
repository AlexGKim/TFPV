...
def process_fullmocks_tf_data(
    ...
    if n_objects is not None and n_objects < N_after_cuts:
        ...
        z_obs = z_obs_sel[idx]
        print(
            f"  Subsampled from {N_after_cuts} to {n_objects} objects for Stan (seed={random_seed})"
        )
    else:
        x, y, sigma_x, sigma_y, z_obs = (
            x_sel,
            y_sel,
            sigma_x_sel,
            sigma_y_sel,
...
def plot_fullmocks_tf_data(
    ...
    if slope_plane is not None and intercept_plane is not None and len(x_all) > 0:
        ...
        ax.plot(
            x_range,
            slope_plane * x_range + intercept_plane,
            "g--",
            linewidth=2,
            alpha=0.8,
        ...
        if intercept_plane2 is not None:
            ax.plot(
                x_range,
                slope_plane * x_range + intercept_plane2,
                "g-.",
                linewidth=2,
                alpha=0.8,
        ...
        if os.path.exists(best_cfg_path):
            ...
            if slope_mle is not None:
                ...
                if slope_mle <= slope_lo + tol or slope_mle >= slope_hi - tol:
                    print(
                        f"  WARNING: _slope_mle={slope_mle:.3f} is at the edge of the "
