...
def load_fullmocks(fits_file):
    ...
    valid = (
        np.isfinite(x_raw)
        & np.isfinite(logvrot_err)
        & np.isfinite(absmag)
        & np.isfinite(absmag_err)
        & np.isfinite(zobs)
        & (logvrot > 0)
        & (logvrot_err > 0)
        & (absmag_err >= 0)
...
def load_desi(fits_file="data/DESI-DR1_TF_pv_cat_v15.fits"):
    ...
    Columns: V_0p4R26, V_0p4R26_ERR, abs_mag_col, abs_mag_err_col, ZOBS.
    ...
    mag = np.asarray(data[col_abs], dtype=float)
    mag_e = np.asarray(data[col_abs_err], dtype=float)
...
def _cuts_at_nsigma(mu, sigma, n_sigma):
    ...
    y_extent = np.sqrt(
        sigma_major**2 * np.sin(angle_rad) ** 2
...
def run_mag_split_grid(
    raw_data, mu, sigma, extra_cuts, exe_file, gmm_slope, perp_vals, lo_vals, hi_vals
):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        for n_sigma_perp in perp_vals:
            ...
            print(
                f"  {'n_σ_lo':>7}  {'n_σ_hi':>7}  {'MLE slope':>10}  {'diff':>8}  {'N':>6}"
            )
            print(f"  {'-' * 7}  {'-' * 7}  {'-' * 10}  {'-' * 8}  {'-' * 6}")
            for nlo in lo_vals:
                for nhi in hi_vals:
                    cuts = _cuts_mag_asymmetric(
                        mu, sigma, float(n_sigma_perp), float(nlo), float(nhi)
                    )
                    cuts.update(extra_cuts)
                    data_dict, init_dict = _build_stan_dicts(raw_data, cuts)
                    if data_dict is None:
                        print(
                            f"  {nlo:7.2f}  {nhi:7.2f}  {'—':>10}  {'—':>8}  {'<30':>6}"
                        )
                    ...
                    if slope is None:
                        print(
                            f"  {nlo:7.2f}  {nhi:7.2f}  {'failed':>10}  {'—':>8}  {N:6d}"
                        )
                        results.append((float(nlo), float(nhi), float("nan"), N))
                        continue
                    diff = slope - gmm_slope
                    print(
                        f"  {nlo:7.2f}  {nhi:7.2f}  {slope:10.4f}  {diff:+8.4f}  {N:6d}"
                    )
...
def plot_mag_split_grid(
    ...
    all_slopes = [
        s for rs in results_by_perp.values() for _, _, s, _ in rs if np.isfinite(s)
    ]
    ...
    abs_max = min(max(abs(d) for d in all_diffs) if all_diffs else 1.0, 2.0 * slope_tol)

    # Diverging symlog norm: linear within ±linthresh, log outside
    linthresh = slope_tol / 4.0
    diff_norm = mcolors.SymLogNorm(
        linthresh=linthresh, vmin=-abs_max, vmax=abs_max, base=10
    )
    ...
    fig, axes = plt.subplots(n_perp, 2, figsize=(11, 4.5 * n_perp), squeeze=False)
    ...
    for row, n_sigma_perp in enumerate(perp_vals):
        ...
        for col, (data, cmap_name, norm, cbar_label) in enumerate(
            [
                (diff_grid, "RdBu", diff_norm, "MLE slope"),
                (N_grid, "viridis", mcolors.Normalize(vmin=N_vmin, vmax=N_vmax), "N"),
            ]
        ):
            ax = axes[row, col]
            ...
            pcm = ax.pcolormesh(
                hi_arr, lo_arr, data, cmap=cmap, norm=norm, shading="nearest"
            )
            ...
            if haty_min_vals is not None and row == n_perp - 1:
                ...
                ax_right.set_yticklabels(
                    [f"{v:.1f}" for v in haty_min_vals], fontsize=7
                )
                ax_right.set_ylabel(r"$\hat{y}_\text{min}$", fontsize=8)

    if fiducial is not None:
        fid_row = int(
            np.argmin(np.abs(np.array(list(perp_vals)) - fiducial["n_sigma_perp"]))
        )
        for col in (0, 1):
            axes[fid_row, col].plot(
                fiducial["n_sigma_hi"],
                fiducial["n_sigma_lo"],
                "*",
                color="black",
...
def plot_fiducial_slope_hist(
    results_by_perp, perp_vals, gmm_slope, slope_tol, run_dir, ref_slope=None
):
...
def find_fiducial_from_mag_split(
    results_by_perp, perp_vals, gmm_slope, slope_tol, contraction=1.0
):
    ...
    mid = [
        (perp, nlo, nhi, slope, N)
        for perp, nlo, nhi, slope, N in good
...
def save_fiducial(fiducial, mu, sigma, run_dir):
    ...
    print(
        f"           haty_min={cuts['haty_min']:.3f}  haty_max={cuts['haty_max']:.3f}"
    )
    print(
        f"           slope_plane={cuts['slope_plane']:.4f}"
...
def save_mag_split_results(results_by_perp, perp_vals, lo_vals, hi_vals, run_dir):
    ...
    out = {
        ...
        "results_by_perp": {str(k): v for k, v in results_by_perp.items()},
    }
...
def load_mag_split_results(run_dir):
    ...
    results_by_perp = {
        float(k): [tuple(row) for row in v] for k, v in data["results_by_perp"].items()
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--exe", default="tophat", help="Path to compiled Stan tophat executable"
    )
    parser.add_argument(
        "--z_obs_min", type=float, default=0.03, help="Minimum redshift cut"
    )
    parser.add_argument(
        "--z_obs_max", type=float, default=0.10, help="Maximum redshift cut"
    )
    parser.add_argument(
        "--n_sweep_objects",
        type=int,
    ...
    parser.add_argument(
        "--n_sigma_perp_min", type=float, default=5.0, help="Lower end of n_σ_perp grid"
    )
    parser.add_argument(
        "--n_sigma_perp_max", type=float, default=5.0, help="Upper end of n_σ_perp grid"
    )
    parser.add_argument(
        "--n_sigma_perp_n", type=int, default=1, help="Number of n_σ_perp grid points"
    )
    parser.add_argument(
        "--n_sigma_mag_lo_min",
        type=float,
    ...
    parser.add_argument(
        "--n_sigma_mag_lo_n", type=int, default=8, help="Number of n_σ_lo grid points"
    )
    parser.add_argument(
        "--n_sigma_mag_hi_min",
        type=float,
    ...
    parser.add_argument(
        "--n_sigma_mag_hi_n", type=int, default=9, help="Number of n_σ_hi grid points"
    )
    ...
    if n_cap and len(raw_data["x"]) > n_cap:
        ...
        raw_data = {
            k: (v[idx] if isinstance(v, np.ndarray) else v) for k, v in raw_data.items()
        }
    ...
    if args.mag_split_plot:
        ...
        fiducial = find_fiducial_from_mag_split(
            ...
            gmm_slope,
            args.slope_tol,
            contraction=args.fiducial_contraction,
        )
        plot_fiducial_slope_hist(
            results_by_perp, perp_vals, gmm_slope, args.slope_tol, run_dir
        )
        plot_mag_split_grid(
            results_by_perp,
            perp_vals,
    ...
    # ── Mag-split grid ───────────────────────────────────────────────────────
    perp_vals = np.linspace(
        args.n_sigma_perp_min, args.n_sigma_perp_max, args.n_sigma_perp_n
    )
    lo_vals = np.linspace(
        args.n_sigma_mag_lo_min, args.n_sigma_mag_lo_max, args.n_sigma_mag_lo_n
    )
    hi_vals = np.linspace(
        args.n_sigma_mag_hi_min, args.n_sigma_mag_hi_max, args.n_sigma_mag_hi_n
    )
    print(
        f"Running mag-split grid: perp={perp_vals}  "
    ...
    if fiducial is None:
        ...
        save_fiducial(fiducial, mu, sigma, run_dir)
    plot_fiducial_slope_hist(
        mag_split_results, perp_vals, gmm_slope, args.slope_tol, run_dir
    )
    plot_mag_split_grid(
        mag_split_results,
        perp_vals,
