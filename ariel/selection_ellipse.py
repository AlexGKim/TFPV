...
def load_data(
    fits_file, haty_min, haty_max, source="fullmocks", z_obs_min=None, z_obs_max=None
):
...
def _unpack(theta):
    ...
    mu1 = theta[1:3]
    L1 = np.array([[np.exp(theta[3]), 0.0], [theta[4], np.exp(theta[5])]])
    mu2 = theta[6:8]
    L2 = np.array([[np.exp(theta[8]), 0.0], [theta[9], np.exp(theta[10])]])
...
def _neg_log_lik(
    theta, xy, sigma2_x, sigma2_y, x_hi, y_lo, mean_sigma2_x, mean_sigma2_y
):
    ...
    p = (
        w1 * _bvn_pdf(xy, mu1, S1, sigma2_x, sigma2_y) / Z1
...
def fit_truncated_gmm(x, y, sigma_x, sigma_y, n_init, x_hi, y_lo):
    ...
    print(
        f"  Optimising noise+truncation-corrected log-likelihood "
    ...
    return _TruncGMM(
        ...
        sigma_x=sigma_x,
        sigma_y=sigma_y,
        mean_sigma2_x=msig2_x,
        mean_sigma2_y=msig2_y,
    )
...
def make_plot(x, y, gmm, core_idx, out_path, x_hi, y_lo):
    ...
    ax.plot(
        x_line,
        cuts3["slope_plane"] * x_line + cuts3["intercept_plane"],
        color="lime",
        lw=2.0,
        ls="-",
    ...
    ax.plot(
        x_line,
        cuts3["slope_plane"] * x_line + cuts3["intercept_plane2"],
        color="lime",
        lw=2.0,
        ls="-",
    ...
    y_pad = 0.3
    ax.set_ylim(
        min(y.min(), cuts3["haty_min"]) - y_pad, max(y.max(), cuts3["haty_max"]) + y_pad
    )
    ax.invert_yaxis()
...
def mle_at_nsigma(
    ...
    data_dict, init_dict = _build_stan_dicts_from_arrays(x, sigma_x, y, sigma_y, cuts)
    ...
    print(
        f"  MLE slope={mle_slope:.4f}  intercept={mle_intercept:.4f}  N={result['N']}"
    )
...
def make_mle_plot(
    x, y, gmm, core_idx, cuts, mle_slope, mle_intercept, n_sigma, out_path, x_hi, y_lo
):
    ...
    # Truncation boundary
    ax.axvline(x_hi, color="white", lw=1.0, ls="-", label=f"x_trunc={x_hi:.3f}")
    ax.axhline(y_lo, color="white", lw=1.0, ls="-", label=f"y_trunc={y_lo:.2f}")
...
def main():
    ...
    parser.add_argument(
        "--file", help="Path to FITS file (required if not using config)"
    )
    parser.add_argument(
        "--run",
        help="Output run directory under output/ (required if not using config)",
    )
    parser.add_argument(
        "--source",
    ...
    if not args.file or not args.run:
        parser.error(
            "The following arguments are required: --file (or 'fits_file' in config), --run"
        )
    ...
    print(
        f"Fitting noise+truncation-corrected 2-component GMM (n_init={args.n_init}) …"
    )
