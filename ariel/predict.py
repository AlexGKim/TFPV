...
def create_average_grid_image(
    ...
    # 1. Compute Grid
    sum_heatmap, xedges, yedges = np.histogram2d(
        x, y, bins=[grid_resolution_x, grid_resolution_y], weights=z
    )
    count_heatmap, _, _ = np.histogram2d(
        x, y, bins=[grid_resolution_x, grid_resolution_y]
    )

    # Calculate average
    average_heatmap = np.divide(
        sum_heatmap,
        count_heatmap,
        where=count_heatmap != 0,
        out=np.nan * np.ones_like(sum_heatmap),
    )
...
def load_xy_and_uncertainties_from_desi(
    ...
    *,
    ...
    with fits.open(fits_path) as hdul:
        ...
        # Resolve mag_col and mag_err_col using centralized logic if not provided
    ...
    if apply_valid_mask:
        ...
        for name, arr in [
            ...
            if np.any(~np.isfinite(arr)):
                raise ValueError(
                    f"Non-finite values found in derived/loaded array '{name}'."
                )
        if np.any(sigma_x < 0):
...
def load_xy_and_uncertainties_from_fullmocks(
    ...
    xhat, sigma_x, yhat, sigma_y, zobs, y_true : np.ndarray
        ...
        sigma_y = R_ABSMAG_SB26_CORR_ERR
        zobs    = ZOBS
    ...
    with fits.open(fits_path) as hdul:
        ...
        absmag_e = np.asarray(data["R_ABSMAG_SB26_CORR_ERR"], dtype=float)
    ...
    *,
    ...
    if y_min is None or y_max is None:
        if bounds_json is None:
            raise ValueError(
                "Provide scalar y_min and y_max, or set bounds_json to a JSON file path."
            )
...
def DESI_normal():
    ...
    mean_pred, sd_pred = ystar_pp_mean_sd_normal_vectorized(
    ...
    fig, ax, img = create_average_grid_image(
        xhat_star, yhat_star, mean_y, grid_resolution_x=50, grid_resolution_y=50
    )
...
def DESI_tophat():
    ...
    mean_pred, sd_pred = ystar_pp_mean_sd_tophat_vectorized(
        draws, xhat_star, sigma_x_star, y_min=-22.5 - 0.1, y_max=-18.5 + 0.1
    )
    mean_y = mean_pred - yhat_star
    sigma_y = sd_pred
    ...
    # redshift plot
    fig, ax, img = create_average_grid_image(
        xhat_star, yhat_star, zobs_star, grid_resolution_x=50, grid_resolution_y=50
    )
    ax.set_xlabel(r"$\log{V/V_0}$")
    ax.set_ylabel(r"$M$")
...
def DESI(
    ...
    # --- posterior + predictive ---
    if kind == "normal":
        mean_pred2, sd_pred2 = ystar_pp_mean_sd_normal_vectorized(
            draws, xhat_star2, sigma_x_star2
        )
    else:
    ...
    if make_residual_grid:
        ...
        img_v = ax_v.imshow(
            ...
            norm=mcolors.LogNorm(
                vmin=np.nanpercentile(_avg, 5), vmax=np.nanpercentile(_avg, 95)
            ),
            interpolation="nearest",
            aspect="auto",
        )
        ...
        _avg_sx = np.divide(
            _sum_sx, _cnt_sx, where=_cnt_sx != 0, out=np.full_like(_sum_sx, np.nan)
        )
    ...
    if kind == "tophat" and compare_tophat_vs_normal and glob.glob(_p("normal_?.csv")):
        ...
        mean_pred_n, sd_pred_n = ystar_pp_mean_sd_normal_vectorized(
            draws_n, xhat_star, sigma_x_star
        )
        mean_y_normal = mean_pred_n - yhat_star
...
def DESI_compare(
    ...
    if log_scale:
        norm = SymLogNorm(
            linthresh=linthresh, linscale=1.0, vmin=vmin, vmax=vmax, base=log_base
        )

        ticks = _symmetric_log_ticks(vmin, vmax, linthresh=linthresh, n_ticks=7)
    ...
    if make_residual_grid:
        ...
        fig.savefig(f"{output_prefix}_diff_grid.png", dpi=300, bbox_inches="tight")
        plt.close(fig)
    ...
    if make_redshift_grid:
        ...
        fig.savefig(
            f"{output_prefix}_redshift_grid_diff.png", dpi=300, bbox_inches="tight"
        )
        plt.close(fig)
    ...
    if make_scatter:
        ...
        plt.scatter(zobs_star, diff, s=3, alpha=0.05, label=r"Top‑Hat – Normal")
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(f"{output_prefix}_redshift_diff.png", dpi=300, bbox_inches="tight")
        plt.close()
...
def ariel(
    ...
    galaxy_csv = "data/TF_mock_tophat-mag_input.csv"
    xhat_star, sigma_x_star, yhat_star, sigma_y_star, zobs_star = (
        load_xy_and_uncertainties_from_csv(galaxy_csv, row=None, sort_by_zobs=True)
    )
    n = zobs_star.shape[0]
    N = 10000
...
def fullmocks(
    ...
    if (
        plane_cut
        and _cfg.get("slope_plane") is not None
        and _cfg.get("intercept_plane") is not None
    ):
        s = _cfg["slope_plane"]
        sel &= (
            yhat_star >= s * xhat_star + _cfg["intercept_plane"] + delta_intercept_plane
        )
        if _cfg.get("intercept_plane2") is not None:
            sel &= (
                yhat_star
                <= s * xhat_star + _cfg["intercept_plane2"] + delta_intercept_plane2
            )
    if sel.sum() < len(sel):
        xhat_star, sigma_x_star, yhat_star, sigma_y_star, zobs_star, y_true = (
            arr[sel]
    ...
    if make_truth_diff_grid:
        ...
        ax.set_ylabel(r"$M$")
        ax.set_title(
            r"$(M_{\text{predicted}} - M_{\text{true}}) / \sigma_{\text{pred}}$"
        )
    ...
    ax.scatter(
        ...
        alpha=0.8,
        color="red",
        label=f"pull > 4  (N = {high_pull.sum()})",
    )
    if _cfg.get("haty_max") is not None:
        ax.axhline(
            _cfg["haty_max"] + delta_haty_max,
            color="red",
            linestyle="--",
            linewidth=1.5,
            label=f"haty_max = {_cfg['haty_max'] + delta_haty_max:.2f}",
        )
    if _cfg.get("haty_min") is not None:
        ax.axhline(
            _cfg["haty_min"] + delta_haty_min,
            color="orange",
            linestyle="--",
            linewidth=1.5,
            label=f"haty_min = {_cfg['haty_min'] + delta_haty_min:.2f}",
        )
    if (
        plane_cut
        and _cfg.get("slope_plane") is not None
        ...
        ax.plot(
            x_range,
            s_p * x_range + _cfg["intercept_plane"] + delta_intercept_plane,
            "g--",
            linewidth=1.5,
        )
        if _cfg.get("intercept_plane2") is not None:
            ax.plot(
                x_range,
                s_p * x_range + _cfg["intercept_plane2"] + delta_intercept_plane2,
                "g-.",
                linewidth=1.5,
            )
    ax.invert_yaxis()
    ...
    z_hist_edges = np.logspace(
        np.log10(zobs_star[finite].min()), np.log10(zobs_star[finite].max()), 10
    )  # 9 bins
    ...
    for b, ax in enumerate(axes):
        ...
        mu, sig = (
            (np.mean(vals), np.std(vals)) if mask_b.sum() > 0 else (np.nan, np.nan)
        )
        ax.set_title(
            f"z ∈ [{z_lo:.3f}, {z_hi:.3f}]  N={mask_b.sum()}\n"
            f"mean={mu:.3f}  std={sig:.3f}",
            fontsize=8,
        )
        ax.set_xlabel("Pull")
...
def write_desi_catalog(model, run_dir, fits_path):
    ...
    new_cols = [
        ...
        fits.Column(
            name="LOGDIST_ERR", format="E", array=LOGDIST_ERR.astype(np.float32)
        ),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Posterior predictions and diagnostics."
    )
    parser.add_argument(
        "--run",
        default=None,
    ...
    if args.source == "DESI":
        ...
        pattern = os.path.join(
            args.dir, f"TF_extended_AbacusSummit_base_{fits_id}_*.fits"
        )
